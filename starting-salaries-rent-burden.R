...
# Variable used: B25064_001 | Estimate!!Median gross rent | Median Gross Rent (Dollars)

# This function pulls median gross rents from ACS 5-year estimates 2018-2023 and compiles them into a single data frame. Note that these 5-year estimates overalap with each other which generally isn't advised, but I manually checked against 1-year estimates and the results were similar - plus 1-year estimates are missing for the year 2020.  
get_rent_data_5 <- function(start_year = 2018, end_year = 2023) {
...
# salary data - l-tables.csv is a CSV downloaded from the manually created 'salary data.gsheet' in this folder. That data is hand-compiled from https://www.lausd.org/page/4045 using the lowest salary for teachers with alternate certifications. See README for more details.
...
starting_salary_rent_plot <- ggplot(starting_salary_rent_data, aes(year, ratio)) +
  ...
  scale_y_continuous(limits = c(35, 43), breaks = seq(0,100, by = 1))
